...
class Recipe(Base):
    ...
    cocktail = relationship(Cocktail, backref=backref(
        'recipes', order_by=order, cascade='all, delete-orphan'))
    ingredient = relationship(Ingredient, backref=backref(
        'recipes', order_by=cocktail_id, cascade='all, delete-orphan'))
...
def getSysRecipes(session):
    ...
    session.sysIngs = {'before': startIng,
                       'after': endIng, 'functions': fct_list}
    if len(fct_list) == 0:
